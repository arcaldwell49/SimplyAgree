ccc.xy <- function(x,y, rep.measure, conf.level) {
  ...
  blalt <- data.frame(x=dat$x,y=dat$y,mean = rmean, delta)
  v <- sd1/sd2
  ...
  res_list <- list(rho.c = rho.c, s.shift = v, l.shift = u,
                   bias = C.b, df_diff = blalt, delta = sblalt,
                   nmissing = nmissing)
