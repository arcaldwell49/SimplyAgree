...
reli_cv_lmer = function(
  ...
  icc_est = res_f$icc[1]
  SEM = sd_tots * sqrt(1-icc_est)
  SEE = sd_tots*sqrt(icc_est*(1-icc_est))
  SEP = sd_tots*sqrt(1-icc_est^2)

  results <- data.frame(matrix(NA, ncol = 1, nrow = 1))
  colnames(results) <- c("model","measures","type", "SE","lower.ci", "upper.ci")
  results$model = res_f$model
  results$measures = res_f$measures
  results$type = se_type
  results$SE = switch(se_type,
                      SEM = SEM,
                      SEP = SEP,
                      SEE = SEE)
  if(conf_type == "chisq"){
    cv_out_ci = cv_ci(cv_out,
                      df = dfE,
                      alpha = alpha)
    results$lower.ci = cv_out_ci[1]
    results$upper.ci = cv_out_ci[2]
    #return(res_f)
  }
  if(conf_type != "chisq"){
    boot_reli <- function(.) {
      icc_boot_loop(.,icc_type = icc_type)
    }
    ...
    res_f$lower.ci = res_other$lower.ci
    res_f$upper.ci = res_other$upper.ci
    res_f$SE = res_other$se
    res_f$bias = res_other$bias
