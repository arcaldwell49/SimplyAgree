...
- delta:
  ...
  guarantees that the expected half-width of the confidence interval
  will be no more than this value. Can be specified in standard
  deviation units.
...
- sigma:
  ...
  value is unknown, delta can be specified in standard deviation units
  by setting sigma = 1.
...
- `delta.actual`: The actual upper bound of expected half-width achieved
  (may differ slightly from target due to discrete nature of n)
...
normal. This function uses the expected half-width criterion to
...
based on expected half-width for Bland-Altman limits of agreement, as
described in Jan and Shieh (2018). The expected half-width criterion
determines an N that guarantees that the expected half-width of the
confidence interval is less than a boundary value delta.
...
in Odeh and Owen (1980) and Hahn and Meeker (1991)), and S is the sample
standard deviation.
...
The expected half-width is E(H) = (g/c) \* sigma. This accounts for the
fact that the sample standard deviation S is a biased estimator of
sigma, requiring correction when computing expected values.
...
E(H) \<= delta. An alternative approach is the assurance probability
criterion (see
[`agree_assurance()`](https://aaroncaldwell.us/SimplyAgree/reference/agree_assurance.md)),
which ensures that Pr(H \<= omega) \>= 1 - gamma.
...
- Is appropriate when a stronger guarantee is needed for planning
  purposes
...
needed. The actual expected half-width may differ slightly from the
...
[`agree_assurance()`](https://aaroncaldwell.us/SimplyAgree/reference/agree_assurance.md)
for sample size determination using assurance probability criterion,
