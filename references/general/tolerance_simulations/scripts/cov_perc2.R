...
out = t(replicate(nsim, {
  ...
  thL = e$emmean - zp*e$SD; thU = e$emmean + zp*e$SD
  qL = quantile((bt$bias - zp*bt$SD - thL)/bt$SD, gam); qU = quantile((bt$bias + zp*bt$SD - thU)/bt$SD, 1 - gam)
  stL = thL - qL*e$SD; stU = thU - qU*e$SD
  ...
  c(joint = cont(j$lower.TL, j$upper.TL) >= beta,
    ...
    st_lo = stL <= mu - zp*sig, st_hi = stU >= mu + zp*sig)
}))
