...
With a variance function (e.g., from `condition`), the analytic limits are approximations and can be somewhat too narrow (coverage of about 93% for a 95% target in our simulations).

The bootstrap-calibrated method (`tol_method = "boot_cal"`) simulates new data from the fitted model, refits the model to each replicate (default is 999 replicates), and computes the analytic limits in each replicate over a range of nominal confidence levels.
...
This corrects the remaining error in the analytic limits, which matters most when a variance function is in the model (e.g., when `condition` is supplied).
For independent data, compound symmetry, or AR(1) correlation alone, the analytic limits were already accurate in our simulations and are much faster.
With very few subjects (around 10), the calibration inherits some error from the estimated correlation and can be slightly liberal.
