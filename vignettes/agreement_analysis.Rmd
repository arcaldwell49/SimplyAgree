...
The prediction interval above is a "beta-expectation" tolerance interval: on average over repeated studies it contains `pred_level` of the differences, but it is not a confidence statement.
The tolerance limits add a confidence statement, and the `bound_type` argument sets which one:

- `bound_type = "joint"` (default): a "beta-content" tolerance interval. With confidence `tol_level`, at least `pred_level` of all differences lie within the limits. If the limits fall within a maximal allowable difference ($\pm \Delta$), we can conclude that at least `pred_level` of differences lie within $\pm \Delta$.
- `bound_type = "iu"`: a one-sided `tol_level` confidence bound on each of the lower and upper limits of agreement (the $(1 \mp \beta)/2$ percentiles of the differences). If both bounds fall within $\pm \Delta$, we can reject that either limit of agreement lies outside $\pm \Delta$ (an intersection-union test). These bounds are not a joint `tol_level` interval, and they target the same quantities as the confidence bounds that `agreement_limit()` reports by default.

The type of tolerance limit calculation can be set using the `tol_method` argument with options "analytic" (default) and "boot_cal".
Both target the interval chosen with `bound_type`.

For `bound_type = "joint"`, the analytic tolerance limits, based on @francq2020tolerate, are calculated as the following:

$$
TI = EMM \pm z_{1-\alpha_1/2} \cdot SEP \cdot \frac{S_U}{S}
$$

where $S$ is the residual standard deviation and $S_U$ is a one-sided upper confidence bound for it, at the `tol_level`.
With independent data, $S_U / S = \sqrt{df / \chi^2_{\alpha_2,df}}$ with $df = n - 1$ (for a model with only an intercept), and this is the approximation of @howe1969.
With repeated measures (compound symmetry), $S_U$ combines the between- and within-subject variance components using the MOVER method [@zou2011], because the between-subject component is estimated with far fewer degrees of freedom than the total.
For other correlation structures, $S_U$ uses an effective degrees of freedom for $S^2$.
When `condition` is supplied (a separate residual variance per condition), the variance components of each condition are bounded using the number of subjects and measurements in that condition.
In our simulations (95% target, 10 to 20 subjects), the analytic limits had coverage of about 95% to 96.5% with compound symmetry (with or without `condition`) and with AR(1) correlation.

The bootstrap-calibrated method (`tol_method = "boot_cal"`) is experimental.
It simulates new data from the fitted model, refits the model to each replicate (default is 999 replicates), and computes the analytic limits in each replicate over a range of nominal confidence levels.
It then reports the analytic limits at the nominal level at which the replicates achieve the target confidence (`lower.TL.level` and `upper.TL.level` in the output).
Because the replicates are simulated from the estimated correlation as if it were known, the calibration has its own error with few subjects: in our simulations with 10 to 15 subjects its coverage ranged from about 93% to 96%.
The analytic limits are therefore recommended; the bootstrap may be useful as a check for models whose analytic limits have not been evaluated (e.g., other variance functions or proportional bias).

### Model assumptions

A few modelling choices deserve attention (see the "Model assumptions" section of `?tolerance_limit` for details):

- With the default marginal model (`model = "gls"`), the autoregressive correlation structures (`cor_type = "ar1"` or `"car1"`) have no persistent subject effect: the correlation between measurements from the same subject decays towards zero over time. If subjects have a persistent bias of their own, these structures understate the uncertainty in the bias and give limits that are too narrow. Setting `model = "lme"` fits a random intercept for each subject and adds the serial correlation to the residuals, which keeps the persistent subject effect (in our simulations the joint tolerance limits had about 89% coverage with `gls` and AR(1), and about 95% with `lme` and AR(1), for a 95% target).
...
- Only one level of clustering (`id`) is supported.
- Under compound symmetry or with `model = "lme"`, the output reports the between- and within-subject components of the standard deviation (`SD.between` and `SD.within`).

For example, a random intercept model with serial correlation of the residuals:
...
tolerance_limit(
  ...
  id = "id",
  time = "trial_num",
  model = "lme", # random intercept for each subject
  cor_type = "ar1" # plus AR(1) correlation of the residuals within subject
)
```
