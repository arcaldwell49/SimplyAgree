...
In the study by Jay and Ravanelli (CITE), they attempted to estimate the effect of varying the time of day (AM or PM) on the measurement of thermoregulatory variables (e.g., rectal and esophageal temperature). In total, participants completed 6 separate trials wherein these variables were measured. While this is a robust study of these variables the analyses focused on ANOVAs and t-tests to determine whether or not the time-of-day (AM or PM). This poses a problem because 1) they were trying to test for equivalence and 2) this is a study of *agreement* not *differences*. Due to the latter point, the use of t-test or ANOVAs (F-tests) is rather inappropriate since they provide an answer to different, albeit related, question.

Instead, the authors could test their hypotheses by using tools that  estimate the absolute *agreement* between the AM and PM sessions within each condition. This is rather complicated because we have multiple measurement within each participant. However, between the tools included in `SimplyAgree` and `cccrm` I believe we can get closer to the right answer.
...
```{r pltsrec,fig.cap="Concordance Plots of Rectal Temperature",echo=FALSE,fig.width=7,fig.height=5}
...
```{r pltseso,fig.cap="Concordance Plots of Esophageal Temperature",echo=FALSE,fig.width=7,fig.height=5}
...
```{r}
knitr::kable(rec.post_loa$bs_tab,
...
df_rec.post %>%
  mutate(Average = (AM+PM)/2) %>%
  ggplot(aes(x=Average,y=diff,color=trial_condition)) +
  # Mean Bias
  geom_hline(yintercept=rec.post_loa$bs_tab$estimate[1], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = rec.post_loa$bs_tab$lower.ci[1],
           ymax = rec.post_loa$bs_tab$upper.ci[1],
           alpha = .5,
           fill = "gray") +
  geom_hline(yintercept=rec.post_loa$bs_tab$estimate[2], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = rec.post_loa$bs_tab$lower.ci[2],
           ymax = rec.post_loa$bs_tab$upper.ci[2],
           alpha = .5,
           fill = "gray") +
  geom_hline(yintercept=rec.post_loa$bs_tab$estimate[3], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = rec.post_loa$bs_tab$lower.ci[3],
           ymax = rec.post_loa$bs_tab$upper.ci[3],
           alpha = .5,
           fill = "gray") +
  geom_point() +
  labs(x = "Average between Trec Post",
       y = "Difference between AM & PM",
       color = "Conditions")
```
...
```{r}
knitr::kable(rec.delta_loa$bs_tab,
...
df_rec.delta %>%
  ...
  ggplot(aes(x=Average,y=diff,color=trial_condition)) +
  # Mean Bias
  geom_hline(yintercept=rec.delta_loa$bs_tab$estimate[1], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = rec.delta_loa$bs_tab$lower.ci[1],
           ymax = rec.delta_loa$bs_tab$upper.ci[1],
           alpha = .5,
           fill = "gray") +
  geom_hline(yintercept=rec.delta_loa$bs_tab$estimate[2], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = rec.delta_loa$bs_tab$lower.ci[2],
           ymax = rec.delta_loa$bs_tab$upper.ci[2],
           alpha = .5,
           fill = "gray") +
  geom_hline(yintercept=rec.delta_loa$bs_tab$estimate[3], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = rec.delta_loa$bs_tab$lower.ci[3],
           ymax = rec.delta_loa$bs_tab$upper.ci[3],
           alpha = .5,
           fill = "gray") +
  geom_point() +
  labs(x = "Average between Trec delta",
       y = "Difference between AM & PM",
       color = "Conditions")
```
...
```{r}
knitr::kable(eso.post_loa$bs_tab,
...
df_eso.post %>%
  ...
  ggplot(aes(x=Average,y=diff,color=trial_condition)) +
  # Mean Bias
  geom_hline(yintercept=eso.post_loa$bs_tab$estimate[1], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = eso.post_loa$bs_tab$lower.ci[1],
           ymax = eso.post_loa$bs_tab$upper.ci[1],
           alpha = .5,
           fill = "gray") +
  geom_hline(yintercept=eso.post_loa$bs_tab$estimate[2], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = eso.post_loa$bs_tab$lower.ci[2],
           ymax = eso.post_loa$bs_tab$upper.ci[2],
           alpha = .5,
           fill = "gray") +
  geom_hline(yintercept=eso.post_loa$bs_tab$estimate[3], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = eso.post_loa$bs_tab$lower.ci[3],
           ymax = eso.post_loa$bs_tab$upper.ci[3],
           alpha = .5,
           fill = "gray") +
  geom_point() +
  labs(x = "Average between Teso Post",
       y = "Difference between AM & PM",
       color = "Conditions")
```
...
```{r}
knitr::kable(eso.delta_loa$bs_tab,
...
df_eso.delta %>%
  ...
  ggplot(aes(x=Average,y=diff,color=trial_condition)) +
  # Mean Bias
  geom_hline(yintercept=eso.delta_loa$bs_tab$estimate[1], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = eso.delta_loa$bs_tab$lower.ci[1],
           ymax = eso.delta_loa$bs_tab$upper.ci[1],
           alpha = .5,
           fill = "gray") +
  geom_hline(yintercept=eso.delta_loa$bs_tab$estimate[2], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = eso.delta_loa$bs_tab$lower.ci[2],
           ymax = eso.delta_loa$bs_tab$upper.ci[2],
           alpha = .5,
           fill = "gray") +
  geom_hline(yintercept=eso.delta_loa$bs_tab$estimate[3], alpha=.75) +
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = eso.delta_loa$bs_tab$lower.ci[3],
           ymax = eso.delta_loa$bs_tab$upper.ci[3],
           alpha = .5,
           fill = "gray") +
  geom_point() +
  labs(x = "Average between Teso delta",
       y = "Difference between AM & PM",
       color = "Conditions")
```
